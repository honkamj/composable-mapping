...
from composable_mapping.dense_deformation import (
    convert_normalized_to_voxel_coordinates,
    convert_voxel_to_normalized_coordinates,
    generate_normalized_coordinate_grid,
...
class CoordinateConversionTests(TestCase):
    ...
    VOXEL_COORDINATES = [tensor([0.0, 4]), tensor([0.0, 2, 10])]
    ...
    def test_conversion_between_coordinates(self) -> None:
        ...
        for voxel_coordinates, normalized_coordinates, shape in zip(
            self.VOXEL_COORDINATES, self.NORMALIZED_COORDINATES, self.SHAPES
        ):
            for (
                voxel_coordinates,
                normalized_coordinates,
            ) in BroadcastShapeTestingUtil.expand_tensor_shapes_for_testing(
                voxel_coordinates, normalized_coordinates
            ):
                assert_close(
    ...
    def test_inverse_consistency(self) -> None:
        ...
        assert_close(
            convert_voxel_to_normalized_coordinates(
                convert_normalized_to_voxel_coordinates(2 * random_coordinates, shape), shape
            ),
...
class CoordinateGridTests(TestCase):
    ...
    def test_coordinate_grid_generation(self) -> None:
        ...
        assert_close(voxel_grid, self.VOXEL_GRID)
        assert_close(normalized_grid, self.NORMALIZED_GRID)
    ...
    def test_consistency_with_conversion(self) -> None:
        ...
        assert_close(convert_voxel_to_normalized_coordinates(voxel_grid), normalized_grid)
        assert_close(convert_normalized_to_voxel_coordinates(normalized_grid), voxel_grid)
    ...
    def test_correct_values_generated(self) -> None:
        ...
        for n_channels in range(1, 3):
            for grid, target in zip(self.POINTS, self.VALUES):
                ...
                for grid, target in BroadcastShapeTestingUtil.expand_tensor_shapes_for_testing(
                    grid, target
                ):
                    if target.ndim > 1:
...
class IntegrateSvfTest(TestCase):
    ...
    def test_correct_output_for_constant_flow(self) -> None:
        ...
        assert_close(integrate_svf(flow), flow)
